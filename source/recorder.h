...
// Si tienes el header del sistema, úsalo, si no, definimos
...
struct VideoManager { uint32_t* granBufferRAM; int framesCapturados; bool grabando; };
...
void EscribirCabeceraAVI(FILE* f, int frames, int ancho, int alto) {
    uint32_t frameSize = ancho * alto * 4; uint32_t dataSize = (frameSize + 8) * frames; uint32_t riffSize = dataSize + 220; 
    Escribir4Chars(f, "RIFF"); EscribirIntLE(f, riffSize); Escribir4Chars(f, "AVI "); Escribir4Chars(f, "LIST"); EscribirIntLE(f, 192); Escribir4Chars(f, "hdrl");
    Escribir4Chars(f, "avih"); EscribirIntLE(f, 56); EscribirIntLE(f, 33333); EscribirIntLE(f, frameSize * 30); EscribirIntLE(f, 0); EscribirIntLE(f, 0x10); 
    EscribirIntLE(f, frames); EscribirIntLE(f, 0); EscribirIntLE(f, 1); EscribirIntLE(f, frameSize); EscribirIntLE(f, ancho); EscribirIntLE(f, alto); EscribirIntLE(f, 0); EscribirIntLE(f, 0); EscribirIntLE(f, 0); EscribirIntLE(f, 0); 
    Escribir4Chars(f, "LIST"); EscribirIntLE(f, 116); Escribir4Chars(f, "strl"); Escribir4Chars(f, "strh"); EscribirIntLE(f, 56); Escribir4Chars(f, "vids"); Escribir4Chars(f, "DIB ");
    EscribirIntLE(f, 0); EscribirIntLE(f, 0); EscribirIntLE(f, 0); EscribirIntLE(f, 1); EscribirIntLE(f, 30); EscribirIntLE(f, 0); EscribirIntLE(f, frames); EscribirIntLE(f, frameSize); EscribirIntLE(f, -1); EscribirIntLE(f, 0);
    EscribirShortLE(f, 0); EscribirShortLE(f, 0); EscribirShortLE(f, ancho); EscribirShortLE(f, alto);
    Escribir4Chars(f, "strf"); EscribirIntLE(f, 40); EscribirIntLE(f, 40); EscribirIntLE(f, ancho); EscribirIntLE(f, alto); EscribirShortLE(f, 1); EscribirShortLE(f, 32);
...
ContextoRecorder IniciarGrabadoraContexto(SDL_Renderer* renderer) {
    ...
    CAMError error = 0;
    CAMStreamInfo streamInfo; streamInfo.type = CAMERA_STREAM_TYPE_1; streamInfo.width = REC_ANCHO; streamInfo.height = REC_ALTO;
...
void GuardarVideoAVI(SDL_Renderer* renderer, VideoManager* vm, TTF_Font* font) {
    ...
    SDL_Surface* txtSurf = TTF_RenderText_Blended(font, "SAVING...", {255, 255, 0, 255});
    if (txtSurf) { SDL_Texture* txtTex = SDL_CreateTextureFromSurface(renderer, txtSurf); SDL_Rect rect = {(1280 - txtSurf->w)/2, 360, txtSurf->w, txtSurf->h}; SDL_RenderCopy(renderer, txtTex, NULL, &rect); SDL_RenderPresent(renderer); SDL_FreeSurface(txtSurf); SDL_DestroyTexture(txtTex); }
    
    time_t t = time(NULL); char nombreArchivo[256]; sprintf(nombreArchivo, "fs:/vol/external01/WiiUCamera Files/Video_%ld.avi", (long)t);
            ...
            for(int i=0; i < vm->framesCapturados; i++) {
                 ...
                 for (int y = REC_ALTO - 1; y >= 0; y--) { 
                     ...
                     for (int x = 0; x < REC_ANCHO; x++) {
                         ...
                         uint32_t r = (pixelOriginal >> 24) & 0xFF; uint32_t g = (pixelOriginal >> 16) & 0xFF; uint32_t b = (pixelOriginal >> 8) & 0xFF; uint32_t a = pixelOriginal & 0xFF;
...
int EjecutarGrabadora(SDL_Renderer* renderer, TTF_Font* font, bool esIngles) {
    ...
    bool enRecorder = true; recFrameListo = false; int resultado = 0;
    ...
    while (enRecorder) {
        ...
        if (vm.grabando) {
            ...
            if (esIngles) DibujarInfoLateral(renderer, font, "Press A: Stop/Save", 170); else DibujarInfoLateral(renderer, font, "Presiona A: Guardar", 170);
        } else {
             if (esIngles) {
                DibujarInfoLateral(renderer, font, "Mode: AVI Video", 50); DibujarInfoLateral(renderer, font, "(A) Start REC", 100); DibujarInfoLateral(renderer, font, "(B) Exit Mode", 140);
             } else {
                DibujarInfoLateral(renderer, font, "Modo: Video AVI", 50); DibujarInfoLateral(renderer, font, "(A) Iniciar REC", 100); DibujarInfoLateral(renderer, font, "(B) Salir Modo", 140);
             }
        }
